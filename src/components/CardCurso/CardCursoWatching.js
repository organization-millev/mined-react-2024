...
const CONTENT_CONTAINER_CLASS = "px-4 py-4 max-h-[92px] ";
...
const CardCurso = ({ title,titleCurso , tag , imageUrl, modulo,clases,duracion,programId,idCurso,isFavorite,idClass,isDetail = true,lessionNombre , idioma }) => {
    ...
    <div  onClick={handleClick} >
        <div className="card-curso-estilo cursor-pointer card-watching max-w-sm rounded overflow-hidden dark:bg-color-dark bg-blanco w-full  lg:!h-auto xl:w-[365px] !h-[213px] shadow-custom-strong rounded-[20px] hidden md:flex md:flex-col hover:text-gris-azulado-profundo group">
            ...
            <div className={CONTENT_CONTAINER_CLASS}>
                <div className={TITLE_CLASS}>  {title} {idioma !== 'ES' && `(${idioma})`} </div>
                {(modulo || clases || duracion) && (
                    <>
                    <div className="font-sans text-small 2xl:text-[13px] font-medium text-gris-oscuro items-center justify-center divide-x divide-gris-oscuro md:pr-[10px] hidden 2xl:flex">
                        <p className="!pr-[16px] 2xl:pr-3 my-2 group-hover:text-gris-azulado-profundo dark:group-hover:text-white">{`Módulo ${modulo}`}</p>
                        <p className="!px-[16px] 2xl:px-3 my-2 group-hover:text-gris-azulado-profundo dark:group-hover:text-white">{`Sesiones ${clases}`}</p>
                        <p className="!px-[16px] 2xl:px-3 my-2 group-hover:text-gris-azulado-profundo dark:group-hover:text-white">{formatDuration(duracion)}</p>
                    </div>
                    ...
                    <div className="font-sans text-small font-medium text-gris-oscuro items-center gap-2 divide-x divide-gris-oscuro flex 2xl:hidden">
                        <p className="!pr-[16px] 2xl:pr-3 my-2  group-hover:text-gris-azulado-profundo dark:group-hover:text-white">{`Mód ${modulo}`}</p>
                        <p className="!px-[16px] 2xl:px-3 my-2 group-hover:text-gris-azulado-profundo dark:group-hover:text-white">{`Sesiones ${clases}`}</p>
                        <p className="!px-[16px] 2xl:px-3 my-2 group-hover:text-gris-azulado-profundo dark:group-hover:text-white">{formatDuration(duracion)}</p>
                    </div>
                    </>
            ...
            <div className="w-3/5  h-[100px] font-sans	py-[7px] flex flex-col gap-2">
                ...
                {(modulo || clases || duracion) && (
                    <div className="font-sans text-[10px] font-medium text-gris-oscuro items-center gap-3 flex ml-2">
                        ...
                        <p className="group-hover:text-gris-azulado-profundo">{`Sesiones ${clases}`}</p>
                        <p className="group-hover:text-gris-azulado-profundo">{formatDuration(duracion)}</p>
                    </div>
                )}
            </div>
        </div> 
