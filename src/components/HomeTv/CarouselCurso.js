...
import "./CarouselCurso.css";
import Slider from 'react-slick';
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import CardCurso from '../CardCurso/CardCursoWatching';
...
const CarouselCurso = ({tituloCarouselCurso, subtitulo, icono, showArrows="true", cursos,isDetail}) => {
    ...
    const navigateAcademy = useNavigate();
    const handleAcademias = () => {
       navigateAcademy('/academias');
    }
    
    const sliderRef = useRef();
    
    const [slidesToShow, setSlidesToShow] = useState(3);
    const [slidesToScroll, setslidesToScroll] = useState(cursos.length >= 3 ? 3 : 1);
    const [initialSlide, setInitialSlide] = useState(cursos.length >= 3 ? 0 : 0.5);
                ...
                <div className="flex flex-col lg:gap-[20px] pl-[5%] lg:px-[10%] 2xl:px-0 2xl:max-w-[1152px] w-full py-7 justify-center mx-auto">
                    ...
                    <div className="w-full flex flex-row">
                        <div className="self-end text-xl font-bold w-1/2 flex flex-col md:flex-row gap-[10px]">
                            ...
                            <div className="font-sans flex flex-row font-bold gap-[8px] cursor-pointer" 
                                 onClick={handleAcademias}>
                                {subtitulo && <p className="text-small dark:text-white">{subtitulo}</p>}
                    ...
                    <Slider ref={sliderRef} {...settings} className="carousel-curso-home">{/*md:h-[250px]*/}
                        {cursos.map((course, index) => (
                            <div key={index} className="h-[240px] lg:!h-[300px]">
